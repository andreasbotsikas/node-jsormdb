...
function JSONDatabase(config) {
	...
	try{
		if (require('path').existsSync(this.config.path)){
			var data = this.fs.readFileSync(this.config.path, 'utf-8');
			if (data) // If the file is not empty
				this.db.insert(JSON.parse(data));
		}else if (this.config.debug)
			console.log("Databse file does NOT exist");
	} catch(err) {
		console.log("The following error occured loading db file " + this.config.path + ":");
		console.log(err);
	}
